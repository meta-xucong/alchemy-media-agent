...
async def signed_output_url(*, output_id: str, source_path: Path) -> str | None:
    if not _enabled():
        return None
    try:
        ...
        return None
    if outputs_root != resolved and outputs_root not in resolved.parents:
        return None

    relative_path = resolved.relative_to(root).as_posix()
    expires = int(time.time()) + max(30, settings.media_acceleration_url_ttl_seconds)
    uri = f"/dl/v2/{quote(relative_path, safe='/')}"
