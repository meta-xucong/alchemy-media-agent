...
const mobileV3HomePreviewRequestTimeoutMs = 30000;
...
const mobileV3OutputsSurfaceCacheKey = "alchemy_mobile_v3_outputs_surface_v1";
const mobileV3PreviewProjectIdsCacheKey = "alchemy_mobile_v3_preview_project_ids_v1";
...
function setupMobileV3Adapter() {
  ...
  document.querySelector("#mobileV3FullPromptCloseBtn")?.addEventListener("click", closeMobileV3FullPrompt);
  document.querySelector("#mobileV3FullPromptDialog")?.addEventListener("click", (event) => {
...
function mobileV3ProjectUsesProfessionalWorkspace(project) {
...
function mobileV3ProjectCountLabel(visibleCount) {
  const loadedCount = Array.isArray(mobileV3State.projects) ? mobileV3State.projects.length : 0;
  const serverTotal = Number(mobileV3State.projectsTotal);
  if (!mobileV3State.projectsHasMore) return `${visibleCount} 个项目`;
  if (Number.isFinite(serverTotal) && serverTotal > loadedCount) {
    return `${visibleCount} 个项目 · 已载入 ${loadedCount}/${Math.floor(serverTotal)}`;
  }
  return `${visibleCount} 个项目 · 还有更多`;
}
...
function openMobileV3VisualAssetLibrary(opener = null) {
  if (mobileV3State.workspaceMode !== "professional") return;
  loadMobileV3VisualAssets({ silent: true, force: true }).catch(() => {});
  ...
  try {
    ...
    for (let index = 0; index < window.localStorage.length; index += 1) {
      ...
      if (
        ...
        || key === mobileV3OutputsSurfaceCacheKey
        || key === mobileV3PreviewProjectIdsCacheKey
        || key.startsWith(`${mobileV3ProjectsCacheKey}:account:`)
        || key.startsWith(`${mobileV3OutputsCacheKey}:account:`)
        || key.startsWith(`${mobileV3OutputsSurfaceCacheKey}:account:`)
        || key.startsWith(`${mobileV3PreviewProjectIdsCacheKey}:account:`)
      ) {
        keys.push(key);
      }
    }
...
function hydrateMobileV3Caches() {
  try {
    ...
    const outputsSurfaceKey = mobileV3CacheStorageKey(mobileV3OutputsSurfaceCacheKey);
    const previewProjectIdsKey = mobileV3CacheStorageKey(mobileV3PreviewProjectIdsCacheKey);
    if (!projectsKey && !outputsKey && !outputsSurfaceKey && !previewProjectIdsKey) return;
    ...
    const outputsSurface = String(window.localStorage.getItem(outputsSurfaceKey) || "").trim();
    const previewProjectIds = JSON.parse(window.localStorage.getItem(previewProjectIdsKey) || "[]");
    ...
    if (Array.isArray(outputs) && outputs.length) {
      ...
      mobileV3State.outputsSurface = outputsSurface === "home_preview" ? "home_preview" : "full";
      mobileV3State.previewProjectIds = outputsSurface === "home_preview" && Array.isArray(previewProjectIds)
        ? new Set(previewProjectIds.map((item) => String(item || "")).filter(Boolean))
        : new Set();
    }
    if (mobileV3State.projects.length) {
...
function persistMobileV3Caches() {
  try {
    ...
    window.localStorage.setItem(outputsSurfaceKey, mobileV3State.outputsSurface || "none");
    window.localStorage.setItem(previewProjectIdsKey, JSON.stringify([...mobileV3State.previewProjectIds]));
  } catch (_error) {
  }
}
...
function mergeMobileV3ProjectItems(primaryItems, fallbackItems) {
  const fallbackById = new Map(
    (Array.isArray(fallbackItems) ? fallbackItems : [])
      .filter((item) => item?.project_id)
      .map((item) => [String(item.project_id), item]),
  );
  return (Array.isArray(primaryItems) ? primaryItems : [])
    .filter((item) => item?.project_id)
    .map((item) => {
      const cached = fallbackById.get(String(item.project_id)) || {};
      const merged = { ...cached, ...item };
      const cachedThumbnails = Array.isArray(cached.latest_thumbnail_urls)
        ? cached.latest_thumbnail_urls.filter(Boolean)
        : [];
...
function mergeMobileV3HomePreviewItems(existingItems, incomingItems) {
  ...
  return Array.from(byProject.values()).sort((a, b) => mobileV3OutputTime(b) - mobileV3OutputTime(a));
}

async function loadMobileV3HomePreviews(projectIds, { append = false } = {}) {
...
function mobileV3ProjectWithResponseMetadata(project, payload) {
  ...
  const hasWorkspace = Object.prototype.hasOwnProperty.call(metadata, "v3_workspace");
  if (!hasEcommerceView && !hasCurrentOperation && !responsePreferences && !hasWorkspace) return project;
  const nextMetadata = { ...(project.metadata || {}) };
  if (hasWorkspace) nextMetadata.v3_workspace = metadata.v3_workspace;
...
async function loadMobileV3Projects({ silent = true, force = false, loadMore = false } = {}) {
  ...
  if (requestingMore) {
    ...
    if (force) {
      ...
      mobileV3State.projectsTotal = null;
  ...
  try {
    ...
    if (requestingMore) {
      ...
      mobileV3State.projects = mergeMobileV3ProjectItems(apiProjects, mobileV3State.projects);
      mobileV3State.outputError = "";
    }
    const previewProjectIds = (requestingMore ? apiProjects : mobileV3State.projects.slice(0, mobileV3ProjectPageSize))
    ...
    const serverTotal = Number(projectsPayload.total);
    mobileV3State.projectsTotal = Number.isFinite(serverTotal) && serverTotal >= 0
      ? Math.floor(serverTotal)
      : mobileV3State.projects.length;
    mobileV3State.projectsLoadError = "";
...
function ensureMobilePageLoadingLayer() {
  let layer = document.querySelector("#mobilePageLoadingLayer");
  if (layer) return layer;
  layer = document.createElement("div");
  layer.id = "mobilePageLoadingLayer";
  layer.className = "v3-mobile-loading-layer mobile-page-loading-layer";
  layer.hidden = true;
  layer.innerHTML = `
    <strong id="mobilePageLoadingTitle">正在加载</strong>
    <small id="mobilePageLoadingDetail">正在读取页面数据。</small>
  `;
  document.body.appendChild(layer);
  return layer;
}

function setMobilePageLoading(visible, title = "", detail = "") {
  const layer = ensureMobilePageLoadingLayer();
  layer.hidden = !visible;
  document.body.classList.toggle("v3-mobile-loading-active", Boolean(visible));
  setText("#mobilePageLoadingTitle", title || "正在加载");
  setText("#mobilePageLoadingDetail", detail || "正在读取页面数据。");
}
...
function mobileV3OutputTime(item) {
...
function mobileV3ProjectFromOutputGroup(projectId, latestItem) {
  ...
  const goal = latestItem?.project_goal || latestItem?.metadata?.project_goal || "";
  return {
    ...
    latest_thumbnail_urls: [mobileV3HomeThumbUrl(latestItem)].filter(Boolean),
...
function mobileV3RecentProjectGroups() {
  ...
  (mobileV3State.outputs || []).forEach((item) => {
    ...
    if (!group.latestItem || mobileV3OutputTime(item) >= mobileV3OutputTime(group.latestItem)) {
...
function mobileV3RecentProjectGroupMap() {
...
function mobileV3ProjectGroupsFromProjects() {
  ...
  const projects = (mobileV3State.projects || [])
    ...
    .filter((project) => mobileV3ProjectUsesProfessionalWorkspace(project) === professional);
...
function mobileV3VisibleProjects() {
  return mobileV3ProjectGroupsFromProjects().map((group) => group.project);
}

function mobileV3FindProject(projectId) {
  if (!projectId) return null;
  return (mobileV3State.projects || []).find((project) => project?.project_id === projectId)
    || mobileV3RecentProjectGroups().find((group) => group.projectId === projectId)?.project
    || null;
}
...
function renderMobileV3ProjectCards({ deferImages = false } = {}) {
  const grid = document.querySelector("#mobileV3ProjectGrid");
  if (!grid) return;
  const groups = mobileV3ProjectGroupsFromProjects();
  const visibleGroups = groups.slice(0, mobileV3State.projectRenderLimit || mobileV3ProjectPageSize);
  setText("#mobileV3ProjectCount", mobileV3ProjectCountLabel(groups.length));
  grid.innerHTML = "";
  grid.classList.toggle("empty-v2-list", groups.length === 0 && !mobileV3State.projectsHasMore);
  if (!groups.length) {
  ...
  visibleGroups.forEach((group) => {
    ...
    card.innerHTML = `
      ...
      <div class="v3-mobile-project-copy">
        ...
        <small>${visualCountLabel} · ${formatDate(latest?.created_at || project.updated_at || project.created_at)}</small>
      </div>
...
function handleMobileV3Click(event) {
  if (event.target.closest("#mobileV3ShowFullPromptBtn")) {
  ...
  if (referenceActionButton) {
    if (referenceActionButton.dataset.mobileV3ReferenceAction === "bind_auto_identity_anchor") {
  ...
  if (outputPromptButton) {
    ...
    const item = mobileV3DisplayOutputsForProject(mobileV3State.currentProject)[index];
    if (item) openMobileV3OutputLightbox(item, { promptOpen: true });
    return;
  }
  ...
  if (removeUploadButton) {
    removeMobileV3ReferenceUpload(Number(removeUploadButton.dataset.mobileV3RemoveUpload));
    return;
  }
  ...
  if (openProjectButton) {
    const project = mobileV3FindProject(openProjectButton.dataset.mobileV3OpenProject);
...
function mobileV3FullPrompt(project = mobileV3State.currentProject) {
  return String(project?.user_goal || project?.goal || project?.short_summary || "").trim();
}

function syncMobileV3PromptFromProject(project = mobileV3State.currentProject, { force = false } = {}) {
  const promptInput = document.querySelector("#mobileV3PromptInput");
  if (!promptInput) return;
  const fullPrompt = mobileV3FullPrompt(project);
  if (!fullPrompt) return;
  const previousProjectPrompt = promptInput.dataset.projectPrompt || "";
  const current = promptInput.value.trim();
  const canReplace = force || !mobileV3State.promptEdited || !current || current === previousProjectPrompt;
  if (canReplace) {
    promptInput.value = fullPrompt;
    promptInput.dataset.projectPrompt = fullPrompt;
    mobileV3State.promptEdited = false;
  }
  updateMobileV3FullPromptButton();
}

function updateMobileV3FullPromptButton() {
  const button = document.querySelector("#mobileV3ShowFullPromptBtn");
  const promptInput = document.querySelector("#mobileV3PromptInput");
  if (!button || !promptInput) return;
  const prompt = (promptInput.value || mobileV3FullPrompt()).trim();
  const overflow = promptInput.scrollHeight > promptInput.clientHeight + 4;
  button.hidden = !(prompt.length > 120 || overflow);
}

function openMobileV3FullPrompt() {
  const dialog = document.querySelector("#mobileV3FullPromptDialog");
  const text = document.querySelector("#mobileV3FullPromptText");
  if (!dialog || !text) return;
  const promptInput = document.querySelector("#mobileV3PromptInput");
  const prompt = (promptInput?.value || mobileV3FullPrompt() || "").trim();
  text.textContent = prompt || "还没有可查看的完整提示词。";
  dialog.hidden = false;
  document.body.classList.add("modal-open");
  document.querySelector("#mobileV3FullPromptCloseBtn")?.focus({ preventScroll: true });
}

function closeMobileV3FullPrompt() {
  const dialog = document.querySelector("#mobileV3FullPromptDialog");
...
function removeMobileV3ReferenceUpload(index) {
  if (!Number.isInteger(index) || index < 0 || index >= mobileV3State.files.length) return;
  mobileV3State.files = mobileV3State.files.filter((_, itemIndex) => itemIndex !== index);
  mobileV3State.uploadedAssets = [];
  mobileV3State.uploadFingerprints = {};
  const input = document.querySelector("#mobileV3ReferenceInput");
  if (input) input.value = "";
  ...
  updateMobileV3Status(
    mobileV3State.files.length
      ? `已移除这张参考图，当前还有 ${mobileV3State.files.length} 张`
      : "参考图已清空",
  );
}
...
function mobileV3PlanningFailureMessage(operation) {
...
function renderMobileV3ProjectGallery(project) {
  ...
  if (openButton && openButton.dataset.mobileV3Bound !== "true") {
    ...
    openButton.addEventListener("click", () => {
      ...
      const target = mobileV3FindProject(projectId) || mobileV3State.currentProject;
...
function renderMobileV3ProjectMeta(project = mobileV3State.currentProject) {
  const chips = document.querySelector("#mobileV3ProjectStyleChips");
  if (!chips) return;
  const styleChips = Array.isArray(project?.memory_summary?.confirmed_style_chips)
    ? project.memory_summary.confirmed_style_chips
    : Array.isArray(project?.latest_context?.confirmed_visual_tone)
      ? project.latest_context.confirmed_visual_tone
      : [];
  chips.innerHTML = "";
  const source = styleChips.length ? styleChips.slice(0, 5) : [mobileV3TemplateLabel(project?.primary_template_id || mobileV3State.selectedTemplate)];
  source.forEach((chip) => {
...
function simplifyMobileV3ProjectHome() {
  const workflowCard = document.querySelector('#mobileV3ProjectDetail [data-mobile-open="v3-workflow"]');
  if (workflowCard) workflowCard.remove();
  const composeCard = document.querySelector('#mobileV3ProjectDetail [data-mobile-open="v3-compose"]');
  if (composeCard) composeCard.classList.add("v3-mobile-primary-entry");
}

function renderMobileV3ProjectSnapshot(project = mobileV3State.currentProject) {
  const node = document.querySelector("#mobileV3ProjectSnapshot");
  if (!node) return;
  if (!project?.project_id) {
    node.classList.add("empty-v2-list");
    node.textContent = "打开项目后，这里会用最简单的方式说明当前状态。";
    return;
  }
  ...
  items.forEach((item) => {
    const card = document.createElement("article");
    card.className = "v3-mobile-snapshot-card";
    card.innerHTML = `<span>${escapeHtml(item.label)}</span><strong>${escapeHtml(item.value)}</strong><small>${escapeHtml(item.note)}</small>`;
...
function renderMobileV3ProjectCurrentOperation(project = mobileV3State.currentProject) {
  ...
  if (planningOperation?.state === "planning_failed" && planningOperation?.terminal === true) {
    ...
    setMobileV3Progress(
      "failed",
      mobileV3PlanningFailureMessage(planningOperation),
    );
    openMobileSurface("v3-project-detail");
    node.hidden = false;
    node.innerHTML = `
      <div>
        ...
        <span>${mobileV3PlanningFailureMessage(planningOperation)}项目历史已保留，也不会自动重复提交。请检查需求和参考后再决定下一步。</span>
      </div>
...
function renderMobileV3EcommerceProjectViewReferences(project, ecommerceView) {
  const board = document.querySelector("#mobileV3ReferenceBoard");
  if (!board) return;
  const originalInputs = mobileV3EcommerceProjectViewItems(ecommerceView, "original_product_inputs");
  const lockedIdentity = mobileV3EcommerceProjectViewItems(ecommerceView, "locked_person_identity");
  const selectedDirections = mobileV3EcommerceProjectViewItems(ecommerceView, "selected_continuation_directions");
  const history = mobileV3EcommerceProjectViewHistory(ecommerceView);
  const sourceLibrary = mobileV3ProjectSourceLibraryEntries(project);
  const autoAnchor = mobileV3ProjectAutoIdentityAnchor(project);
  ...
  setText("#mobileV3ReferenceCount", `原图 ${originalInputs.length} · 人物 ${lockedIdentity.length} · 成片 ${selectedDirections.length} · 锚点 ${autoAnchor ? 1 : 0}`);
  board.innerHTML = "";
  board.classList.toggle("empty-v2-list", totalCount === 0);
  if (!totalCount) {
    board.textContent = "还没有原始商品图、人物资产、延续方向或复核记录。";
    return;
  }
  ...
  const renderImageGroup = ({ key, title, description, refs, sourceKind }) => {
    ...
    group.className = `v3-mobile-reference-group ${escapeHtml(key)}`;
    group.innerHTML = `
    ...
    refs.forEach((item, index) => {
      ...
      tile.innerHTML = `
        ...
        <div class="v3-mobile-reference-copy">
          ...
          <small>${escapeHtml(sourceUnavailable ? "需处理" : sourceEntry ? "已验证" : note)}</small>
          <div class="v3-mobile-reference-actions">
            <button type="button" data-mobile-v3-reference-action="remove" data-mobile-v3-reference-id="${escapeHtml(referenceId)}" data-mobile-v3-output-id="${escapeHtml(outputId)}" data-mobile-v3-reference-source="${escapeHtml(ref.source_type)}">${removalLabel}</button>
          </div>
        </div>
      `;
  ...
  renderImageGroup({
    ...
    title: "原始商品图",
    description: "只放用户上传并被服务器确认为商品事实的原图。",
    refs: originalInputs,
  ...
  if (lockedIdentity.length) {
    const group = document.createElement("section");
    group.className = "v3-mobile-reference-group locked_person_identity";
    group.innerHTML = `
      <div class="v3-mobile-reference-group-head">
        <div>
          <strong>人物视觉资产</strong>
          <small>人物资产负责一致性，不会被当作商品原图。</small>
        </div>
  ...
  renderImageGroup({
    ...
    title: "已选延续方向",
    description: "只来自确认过的项目成片，不覆盖原始商品事实。",
  ...
  if (historyCount) {
    const group = document.createElement("section");
    group.className = "v3-mobile-reference-group generated_and_review_history";
    const rows = [
      { label: "正式成果", value: `${history.delivered_outputs.length} 张`, note: "下方项目成果查看" },
      { label: "复核记录", value: `${history.review_withheld_outputs.length} 张`, note: "仅供人工确认" },
      { label: "失败尝试", value: `${history.failed_attempts.length} 次`, note: "不展示内部细节" },
    ];
    group.innerHTML = `
      <div class="v3-mobile-reference-group-head">
        <div>
          <strong>生成与复核历史</strong>
          <small>正式图、复核图和失败尝试分开记录。</small>
        </div>
        <span>${historyCount} 条</span>
      </div>
      <div class="v3-mobile-ecommerce-history-summary">
        ${rows.map((row) => `
          <article>
...
function renderMobileV3ReferenceBoard(project = mobileV3State.currentProject) {
  ...
  const referenceCount = (sourceLibrary.length || groups.original_inputs.length)
    ...
    + (autoAnchor ? 1 : 0);
  setText("#mobileV3ReferenceCount", `原图 ${groups.original_inputs.length} · 成片 ${groups.continuation_outputs.length} · 锚点 ${autoAnchor ? 1 : 0}`);
  board.innerHTML = "";
...
async function bindMobileV3AutoIdentityAnchor(anchor) {
  const projectId = mobileV3State.currentProject?.project_id;
  const outputId = String(anchor?.output_id || "").trim();
  const projectAnchor = mobileV3ProjectAutoIdentityAnchor(mobileV3State.currentProject);
  const jobId = String(anchor?.job_id || projectAnchor?.job_id || "").trim();
  if (!projectId || !outputId || !jobId) {
    updateMobileV3Status("这张锚点图暂时不能绑定。");
    return;
  }
  try {
    setMobileV3Busy(true);
    ...
    mobileV3State.currentJob = payload.job_status || mobileV3State.currentJob;
    await refreshMobileV3ProjectDetail(projectId);
    updateMobileV3Status("已绑定人物身份锚点。");
  } catch (error) {
    updateMobileV3Status(`绑定失败：${friendlyError(error)}`);
  } finally {
    setMobileV3Busy(false);
  }
}
...
function renderMobileV3Timeline(items = mobileV3State.currentTimeline) {
  const node = document.querySelector("#mobileV3Timeline");
  if (!node) return;
  node.innerHTML = "";
  node.classList.toggle("empty-v2-list", !items.length);
  if (!items.length) {
    node.textContent = "项目开始后，关键动作会显示在这里。";
    return;
  }
  items.slice(0, 12).forEach((item) => {
    const row = document.createElement("article");
    row.className = "v3-mobile-timeline-row";
    row.innerHTML = `
      <strong>${escapeHtml(mobileV3TimelineTitle(item))}</strong>
      <span>${escapeHtml(mobileV3TimelineNote(item))}</span>
      <small>${escapeHtml(formatDate(item.created_at || item.timestamp || item.updated_at))}</small>
    `;
...
function openMobileV3OutputLightbox(item, { promptOpen = false } = {}) {
  openImageLightbox({
...
function mobileV3EcommerceProjectView(project = mobileV3State.currentProject) {
...
function mobileV3EcommerceProjectViewGroup(view, key) {
  const group = view?.groups?.[key];
  return group && typeof group === "object" ? group : {};
}

function mobileV3EcommerceProjectViewItems(view, key) {
  const items = mobileV3EcommerceProjectViewGroup(view, key).items;
  return Array.isArray(items) ? items.filter(Boolean) : [];
}

function mobileV3EcommerceProjectViewHistory(view) {
  const group = mobileV3EcommerceProjectViewGroup(view, "generated_and_review_history");
  return {
...
function mobileV3EcommerceProjectReferenceGroups(project = mobileV3State.currentProject) {
  const view = mobileV3EcommerceProjectView(project);
  if (!view) return null;
  const original_inputs = mobileV3EcommerceProjectViewItems(view, "original_product_inputs").map((item) => ({
    ...item,
    source_type: "uploaded",
    use_policy: "product",
    label: item.label || "原始商品图",
  }));
  const continuation_outputs = mobileV3EcommerceProjectViewItems(view, "selected_continuation_directions").map((item) => ({
    ...item,
    source_type: "generated_selected",
    asset_ref_id: item.output_id || item.asset_ref_id,
    created_from_output_id: item.output_id,
...
function renderMobileV3ProjectSourceLibraryGroup(board, entries) {
  ...
  const group = document.createElement("section");
  group.className = "v3-mobile-reference-group project_source_library";
  group.innerHTML = `
    <div class="v3-mobile-reference-group-head">
      <div>
        <strong>项目原始素材</strong>
        <small>当前项目上传的原图目录；人物资产、延续方向和历史记录保持分开。</small>
      </div>
  ...
  entries.forEach((entry, index) => {
    ...
    const tile = document.createElement("article");
    tile.className = "v3-mobile-reference-tile v3-mobile-source-library-tile";
    tile.dataset.mobileV3SourceLibraryAssociation = String(entry.association_reference_id);
    tile.innerHTML = `
      <div class="v3-mobile-reference-thumb"><span>原图</span></div>
      <div class="v3-mobile-reference-copy">
        <span class="v3-mobile-reference-origin">${available ? "已验证原始素材" : "原始素材需处理"}</span>
        <strong>${escapeHtml(entry.label || `项目原始素材 ${index + 1}`)}</strong>
        <small>${escapeHtml(states[entry.availability_state] || "需处理")}</small>
      </div>
    `;
    grid?.appendChild(tile);
  });
...
function mobileV3ProjectReferenceGroups(project = mobileV3State.currentProject) {
  const ecommerceGroups = mobileV3EcommerceProjectReferenceGroups(project);
...
function mobileV3ProjectAutoIdentityAnchor(project = mobileV3State.currentProject) {
...
function mobileV3TimelineTitle(item) {
  return item?.title || item?.label || item?.action || item?.event_type || "项目动作";
}

function mobileV3TimelineNote(item) {
  return item?.summary || item?.note || item?.detail || item?.description || "项目记录";
}
...
function mobileV3HomeThumbUrl(item) {
  return mobileV3MediaUrl(
    item?.thumbnail_url
...
function openMobileSurface(id, opener = null) {
  ...
  if (id === "v3-compose") {
    syncMobileV3PromptFromProject(mobileV3State.currentProject);
    window.requestAnimationFrame(updateMobileV3FullPromptButton);
  }
...
async function initV2({ silent = true, force = false } = {}) {
  ...
  try {
    ...
    if (showPageOverlay) setMobilePageLoading(false);
  }
}
...
async function refreshHistory({ silent = false, append = false } = {}) {
  ...
  if (showPageOverlay) setMobilePageLoading(true, "正在加载 V1", "正在读取最近生成记录和缩略图。");
  try {
