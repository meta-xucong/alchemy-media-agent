...
class OpenAIGPTImageProvider:
    ...
    def _provider_reference_paths(self, reference_paths: list) -> list:
        return [self._provider_reference_path(path) for path in reference_paths]
    ...
    def _provider_reference_path(self, path):
        try:
            ...
            if source.stat().st_size <= max_bytes and source.suffix.lower() in {".jpg", ".jpeg", ".webp", ".png"}:
                return source
            return self._compressed_reference_path(source, max_bytes=max_bytes)
        except Exception:
            return path

    def _compressed_reference_path(self, source: Path, *, max_bytes: int) -> Path:
        try:
