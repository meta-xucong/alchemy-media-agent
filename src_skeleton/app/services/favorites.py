...
def list_favorite_ids(*, veyra_user_id: int | None = None, include_legacy_public: bool = True) -> set[str]:
    ...
    for item in payload.get("items", []):
        ...
        if veyra_user_id is not None and owner_id not in {veyra_user_id, None if include_legacy_public else -1}:
            continue
...
def set_favorite(output_id: str, favorite: bool, *, veyra_user_id: int | None = None) -> dict[str, Any]:
    ...
    _write_payload({"items": sorted(kept, key=lambda item: (str(item.get("output_id") or ""), str(item.get("veyra_user_id") or "")))})
