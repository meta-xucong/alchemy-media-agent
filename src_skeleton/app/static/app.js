...
async function initV3Shell({ force = false } = {}) {
  ...
  try {
    ...
    if (!Array.isArray(payload.templates)) throw new Error("template_catalog_unavailable");
    v3State.templates = payload.templates;
    v3State.templatesLoaded = true;
    v3State.templatesError = "";
    ...
    v3State.templates = [];
    v3State.templatesLoaded = false;
    v3State.templatesError = "项目类型暂时无法读取";
    v3State.projectsLoaded = true;
    v3State.loaded = true;
    renderV3HomeTemplateChooser();
    renderV3Projects();
...
function v3TemplateCanCreate(templateId) {
  if (!v3State.templatesLoaded || v3State.templatesError) return false;
  const templates = v3State.templates;
...
function v3TemplatePlainLabel(templateId) {
  ...
  return template?.display_name || "项目类型待确认";
}
...
function renderV3HomeTemplateChooser() {
  ...
  if (!v3State.templatesLoaded && !v3State.templatesError) {
  ...
  if (v3State.templatesError) {
    els.v3TemplateChooser.innerHTML = `
      <div class="v3-template-catalog-error" role="status">
        <strong>项目类型暂时无法读取</strong>
        <small>为避免创建到错误模板，请重新读取后再创建项目。</small>
        <button class="button compact secondary" type="button" data-v3-template-catalog-retry>重新读取</button>
      </div>`;
    if (els.v3NewProjectBtn) els.v3NewProjectBtn.disabled = true;
    return;
  }
...
function handleV3HomeTemplateChoice(event) {
  const retry = event.target.closest("[data-v3-template-catalog-retry]");
  if (retry) {
    loadV3Projects({ silent: false, force: true });
    return;
  }
...
async function createV3Project() {
  ...
  if (!v3State.templatesLoaded || v3State.templatesError) {
    showGlobalToast("项目类型暂时无法读取，请重新读取后再创建。", "warning");
    return;
  }
  if (!userGoal) {
...
function v3ProjectNextSuggestion(project = v3State.currentProject) {
  ...
  if (!hasImages) return v3State.selectedScenario === "ecommerce" ? "打开制作页，生成第一组电商图片" : "打开制作页，生成第一张图片";
  ...
  items.slice(0, 6).forEach((item, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="v3-output-actions">
        <button type="button" data-v3-output-action="prompt" data-v3-output-index="${index}">提示词</button>
...
function renderV3ProductionEntry({ project, imageCount, refCount, avoidCount, isEcommerce }) {
  ...
  const title = isEcommerce ? "继续生成电商图片" : "继续生成图片";
  const body = refCount
    ? `沿用 ${refCount} 个已选参考继续。`
    : "先生成，再挑满意方向。";
...
function v3ProjectSubpageCopy(stepKey = "compose") {
  ...
  const map = {
    compose: {
      eyebrow: "继续生产",
      title: ecommerceActive ? "继续生成电商图片" : "继续生成图片",
