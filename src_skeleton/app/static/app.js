...
const v3HomePreviewRequestTimeoutMs = 30000;
...
const els = {
  ...
  v3CreateVisualAssetShortcutBtn: document.querySelector("#v3CreateVisualAssetShortcutBtn"),
  v3VisualAssetLibrarySummary: document.querySelector("#v3VisualAssetLibrarySummary"),
  v3VisualAssetLibraryDialog: document.querySelector("#v3VisualAssetLibraryDialog"),
  ...
  v3CountHint: document.querySelector("#v3CountHint"),
  v3GenerationModeDetails: document.querySelector("#v3GenerationModeDetails"),
  ...
  v3EcommerceSellingPointsInput: document.querySelector("#v3EcommerceSellingPointsInput"),
...
function bindControls() {
  ...
  if (els.v3OpenVisualAssetLibraryBtn) els.v3OpenVisualAssetLibraryBtn.addEventListener("click", () => openV3VisualAssetLibraryDialog());
  ...
  if (els.v3CountInput) {
    ...
    els.v3CountInput.addEventListener("change", syncCount);
    els.v3CountInput.addEventListener("input", syncCount);
  }
...
async function initV3Shell({ force = false } = {}) {
  ...
  try {
    ...
    const serverProjectIds = new Set(apiProjects.map((item) => String(item?.project_id || "")).filter(Boolean));
    v3State.projects = mergeV3ProjectItems(
      apiProjects,
      localProjects.filter((item) => serverProjectIds.has(String(item?.project_id || ""))),
    );
...
function v3TemplateDisplayDescription(template) {
  if (template?.template_id === "ecommerce_template") {
    return "上传商品图、说明想要的画面并选择数量；每张图片都按本次需求单独整理。";
  }
  return template?.description || "选择后再创建项目。";
}
...
function v3HomeTemplateCopy(templateId) {
  ...
  if (templateId === "ecommerce_template") {
    return {
      title: "电商创意项目",
      intro: "上传商品参考图、说清想要的画面，再选择数量。系统会为每张图整理完整画面方向，并优先保持实物一致。",
      goalLabel: "这次商品想呈现什么效果",
      goalHint: "先写一句目标即可。平台、市场和已确认的商品事实可在项目里作为补充信息填写。",
      button: "创建电商创意项目",
      placeholder: "例如：参考这款饮品，做明亮清爽、有夏日感的上架商品图片",
    };
  }
  return {
    title: "通用创意项目",
...
function renderV3HomeTemplateChooser() {
  ...
  templates.forEach((template) => {
    ...
    card.innerHTML = `
      ...
      <small>${escapeHtml(v3TemplateDisplayDescription(template))}</small>
    `;
...
function v3ScenarioNotice(scenarioId) {
  if (scenarioId === "ecommerce") {
    return "电商创意已就绪：上传商品图、写一句需求并选择数量；平台信息可按需补充。";
  }
...
function v3ScenarioWorkspaceCopy(scenarioId = "general_creative") {
  ...
  if (ecommerce) {
    return {
      eyebrow: "电商特调",
      title: "电商特调 Agent",
      intro: "上传商品参考图、写一句想要的画面并选择数量。每张图都会由本次需求决定，不预设固定图组。",
      promptLabel: "这次商品图想呈现什么效果",
      promptPlaceholder: "例如：参考商品图，做一张自然明亮、适合夏季上架的饮品图片",
      promptHint: "一句自然语言即可；平台、市场和商品事实可在补充信息中按需填写。",
      assetTitle: "上传商品图",
      assetEmpty: "上传商品参考图后更容易保持实物一致",
      assetSummaryEmpty: "建议上传清晰、可确认的商品实拍图；其他参考图可作为补充。",
      ...
      generatedNotice: "电商图片已生成，可以查看并继续使用满意的结果。",
...
function renderV3ScenarioState() {
  ...
  if (els.v3GenerationModeDetails) {
    els.v3GenerationModeDetails.hidden = selected === "ecommerce";
    if (selected === "ecommerce") els.v3GenerationModeDetails.open = false;
  }
...
async function loadV3ProjectOutputs({
  ...
  try {
    ...
    const displayItems = [...items, ...historyItems];
    if (!scopedProjectId) {
      const appendPreviewItems = normalizedSurface === "home_preview"
        && appendHomePreview
        && v3State.imageHistorySurface === "home_preview";
      v3State.imageHistory = appendPreviewItems
        ? mergeV3HomePreviewItems(v3State.imageHistory, displayItems)
        : displayItems;
      ...
      if (normalizedSurface === "home_preview") {
        syncV3HomeProjectProjection({
          projectIds: requestedHomeProjectIds,
    ...
    if (scopedProjectId) {
      syncV3ProjectOutputsFromList(items, scopedProjectId, reviewItems, historyItems);
    }
    if (!v3ProjectDetailRequestIsCurrent(scopedProjectId, detailEpoch, shouldContinue)) return [];
    renderV3History();
    renderV3HeroHistory();
...
function clearV3LocalCaches() {
  try {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      if (!key) continue;
      if (
        key === v3ProjectStorageKey
        || key === v3HistoryStorageKey
        || key.startsWith(`${v3ProjectStorageKey}:account:`)
        || key.startsWith(`${v3HistoryStorageKey}:account:`)
      ) {
        keys.push(key);
      }
    }
    ...
    // V3 still treats the server response as authoritative when storage is unavailable.
  }
}

function readV3LocalProjects() {
  try {
    const projectKey = v3CacheStorageKey(v3ProjectStorageKey);
    const historyKey = v3CacheStorageKey(v3HistoryStorageKey);
    if (!projectKey && !historyKey) return [];
    const raw = localStorage.getItem(projectKey) || localStorage.getItem(historyKey);
...
function writeV3LocalProjects(items) {
  try {
    const key = v3CacheStorageKey(v3ProjectStorageKey);
    if (!key) return;
    localStorage.setItem(key, JSON.stringify(items.slice(0, v3ProjectCacheLimit)));
  } catch {
...
function syncV3ProjectOutputsFromList(items, projectId = v3State.currentProject?.project_id, reviewItems = null, historyItems = null) {
...
function syncV3HomeProjectHistoryCounts(counts) {
  if (!counts || typeof counts !== "object" || !Array.isArray(v3State.projects)) return;
  let changed = false;
  v3State.projects = v3State.projects.map((project) => {
    const projectId = String(project?.project_id || "").trim();
...
function syncV3HomeProjectReviewCounts(counts) {
  if (!counts || typeof counts !== "object" || !Array.isArray(v3State.projects)) return;
  let changed = false;
  v3State.projects = v3State.projects.map((project) => {
    const projectId = String(project?.project_id || "").trim();
    ...
    return {
      ...
      // The response evaluated this project.  Do not retain a review cover
      // from an older home response when the current projection has no safe
      // review pixel (the cover sync below may add the new one back).
...
function syncV3HomeProjectReviewCovers(items) {
  if (!Array.isArray(items) || !Array.isArray(v3State.projects)) return;
  const covers = new Map();
  items.forEach((item) => {
    const projectId = String(item?.project_id || item?.metadata?.project_id || "").trim();
...
function syncV3HomeProjectProjection({
  projectIds = null,
  formalCounts = null,
  historyCounts = null,
  reviewCounts = null,
  ...
  if (!Array.isArray(v3State.projects)) return;
...
function markV3HomePreviewStale(projectIds = null) {
  if (!Array.isArray(v3State.projects)) return;
  const targetIds = new Set(
    Array.isArray(projectIds)
      ? projectIds.map((value) => String(value || "").trim()).filter(Boolean)
      : v3State.projects.map((project) => String(project?.project_id || "").trim()).filter(Boolean),
  );
  if (!targetIds.size) return;
  v3State.projects = v3State.projects.map((project) => (
...
function mergeV3ProjectItems(primaryItems, fallbackItems) {
  ...
  [...fallbackItems, ...primaryItems].forEach((item) => {
    ...
    if (existing.history_output_count_known === true && item.history_output_count_known !== true) {
      merged.history_output_count = existing.history_output_count;
      merged.history_output_count_known = true;
    }
    if (existing.review_output_count_known === true && item.review_output_count_known !== true) {
      merged.review_output_count = existing.review_output_count;
      merged.review_output_count_known = true;
    }
    const cachedReviewThumbnails = Array.isArray(existing.latest_review_thumbnail_urls)
      ? existing.latest_review_thumbnail_urls.filter(Boolean)
      : [];
...
function appendV3ProjectItems(existingItems, incomingItems) {
  const seen = new Set();
  const result = [];
  [...(Array.isArray(existingItems) ? existingItems : []), ...(Array.isArray(incomingItems) ? incomingItems : [])]
    .forEach((item) => {
...
function v3ProjectEmptyImageLabel(project) {
  if (project?.home_preview_stale === true) {
    return "图片暂时无法读取";
  }
  if (v3ProjectReviewOutputCountKnown(project) && v3ProjectReviewOutputCount(project) > 0) {
    return "待复核图片已找到";
  }
  if (v3ProjectHistoryOutputCountKnown(project) && v3ProjectHistoryOutputCount(project) > 0) {
    return "历史图片已找到";
  }
...
function v3ProjectImageCountLabel(group) {
  const formalKnown = v3ProjectVisibleOutputCountKnown(group?.project);
  const formalCount = v3ProjectVisibleOutputCount(group?.project);
  const historyCount = v3ProjectHistoryOutputCount(group?.project);
  const historyKnown = v3ProjectHistoryOutputCountKnown(group?.project);
  const reviewKnown = v3ProjectReviewOutputCountKnown(group?.project);
  const reviewCount = v3ProjectReviewOutputCount(group?.project);
  const fullSurface = v3State.imageHistorySurface === "full" && v3State.imageHistoryLoaded;
  ...
  if (v3State.imageHistoryError && !hasKnownCount) {
    return "图片暂时无法读取";
  }
  const parts = [];
...
function v3ProjectHistoryOutputCount(project) {
  const raw = project?.history_output_count ?? project?.memory_summary?.history_output_count;
  const value = Number(raw);
  return Number.isFinite(value) && value >= 0 ? Math.floor(value) : 0;
}

function v3ProjectHistoryOutputCountKnown(project) {
  const hasDirectValue = Boolean(
    project
...
function v3ProjectReviewOutputCount(project) {
  const raw = project?.review_output_count ?? project?.memory_summary?.review_output_count;
  const value = Number(raw);
  return Number.isFinite(value) && value >= 0 ? Math.floor(value) : 0;
}

function v3ProjectReviewOutputCountKnown(project) {
  const hasDirectValue = Boolean(
    project
...
function v3ProjectImageCountText(project) {
  ...
  const formalKnown = v3ProjectVisibleOutputCountKnown(project);
  const formalCount = v3ProjectVisibleOutputCount(project);
  const historyKnown = v3ProjectHistoryOutputCountKnown(project);
  const historyCount = v3ProjectHistoryOutputCount(project);
  const reviewKnown = v3ProjectReviewOutputCountKnown(project);
  const reviewCount = v3ProjectReviewOutputCount(project);
  const parts = [];
  if (formalCount > 0 || (formalKnown && historyCount === 0 && reviewCount === 0)) {
    parts.push(`${formalCount} 张图片`);
  }
  if (historyCount > 0) parts.push(`${historyCount} 张历史`);
  if (reviewCount > 0) parts.push(parts.length ? `${reviewCount} 张待复核` : `${reviewCount} 张待复核图片`);
  if (parts.length) return parts.join(" · ");
  if (formalKnown) return `${formalCount} 张图片`;
  if (historyKnown || reviewKnown) return "图片数量未同步";
...
function renderV3Projects() {
  ...
  items.slice(0, v3State.projectRenderLimit).forEach((item) => {
    ...
    const thumbnails = v3ProjectHomeThumbnailUrls(item).slice(0, 1);
  ...
  if (v3State.projectsHasMore || items.length > v3State.projectRenderLimit) {
    ...
    loadMore.className = "v3-project-card v3-project-load-more-card";
    loadMore.innerHTML = `
      <div class="v3-project-thumb-wrap v3-project-load-more-thumb">
        <span>${v3State.projectsHasMore ? "…" : Math.max(0, items.length - v3State.projectRenderLimit)}</span>
      </div>
      <div class="v3-project-goal-slot">
        <p>还有更多项目可以查看</p>
      </div>
      <div class="v3-history-meta">
        <span>首页先显示两排，继续加载不会一次拉大图</span>
      </div>
      <div class="v3-project-card-actions">
        <button type="button" class="button compact secondary" data-v3-project-action="load_more_projects"${v3State.projectsLoadingMore ? " disabled" : ""}>${v3State.projectsLoadingMore ? "加载中..." : "加载更多项目"}</button>
      </div>
    `;
    els.v3ProjectList.appendChild(loadMore);
  }
}
...
function renderV3History() {
  ...
  if (v3State.projectsHasMore || groups.length > visibleGroups.length) {
    ...
    loadMore.innerHTML = `
      <button class="v3-history-project-preview v3-history-load-more-preview" type="button" data-v3-history-load-more aria-label="加载更多项目">
      ...
      <div class="v3-history-actions">
        <button type="button" class="button compact secondary" data-v3-history-load-more>加载更多</button>
      </div>
    `;
    els.v3HistoryList.appendChild(loadMore);
  }
}
...
function v3OutputProjectGroupMap(items = v3State.imageHistory) {
  ...
  v3HistoryDisplayItems(Array.isArray(items) ? items : []).forEach((item) => {
    ...
    group.count += 1;
    if (v3HistoryOnlyOutputVisible(item)) group.historyCount += 1;
    else group.formalCount += 1;
  ...
  return new Map(Array.from(groups.values())
    .map((group) => ({
      ...group,
...
function v3ProjectThumbnailItem(project) {
  const thumbnails = v3ProjectHomeThumbnailUrls(project);
  const formalThumbnails = v3ProjectHomeFormalThumbnailUrls(project);
...
function v3ProjectHomeFormalThumbnailUrls(project) {
  const thumbnails = Array.isArray(project?.latest_thumbnail_urls)
    ? project.latest_thumbnail_urls.filter(Boolean)
    : [];
  const formalEmpty = v3ProjectVisibleOutputCountKnown(project)
    && v3ProjectVisibleOutputCount(project) === 0
    && v3ProjectHistoryOutputCountKnown(project)
    && v3ProjectHistoryOutputCount(project) === 0;
  return formalEmpty ? [] : thumbnails;
}

function v3ProjectHomeThumbnailUrls(project) {
  const formalThumbnails = v3ProjectHomeFormalThumbnailUrls(project);
  const reviewThumbnails = Array.isArray(project?.latest_review_thumbnail_urls)
    ? project.latest_review_thumbnail_urls.filter(Boolean)
    : [];
...
function v3ProjectGroupFromProject(project, outputGroup = null) {
  ...
  const visibleOutputCount = v3ProjectVisibleOutputCount(project);
  const historyOutputCount = v3ProjectHistoryOutputCount(project);
  const reviewOutputCount = v3ProjectReviewOutputCount(project);
  return {
...
function v3ProjectImageGroups(items = v3State.imageHistory) {
  ...
  return projects.map((project) => v3ProjectGroupFromProject(project, outputGroups.get(String(project.project_id))))
    .sort((a, b) => {
      const time = (Date.parse(b.latestAt || "") || 0) - (Date.parse(a.latestAt || "") || 0);
      return time || v3ProjectTime(b.project) - v3ProjectTime(a.project);
    });
}
...
function v3HistoryOnlyOutputVisible(item) {
  if (v3FailureArtifactExpired(item)) return false;
  if (!item?.history_only && !item?.metadata?.history_only && !item?.metadata?.legacy_history_only) return false;
  return v3OutputImageCandidates(item).length > 0;
}

function v3HistoryDisplayItems(items = []) {
  return v3DedupeOutputItems(items).filter(
    (item) => v3HistoryOutputVisible(item) || v3HistoryOnlyOutputVisible(item),
  );
}
...
function v3ProjectImageGroup(projectId) {
  ...
  const scopedItems = Array.isArray(v3State.projectOutputs)
    ? v3State.projectOutputs.filter((item) => String(item?.project_id || item?.metadata?.project_id || "") === cleanProjectId)
    : [];
...
function renderV3ProjectDetail() {
  ...
  if (els.v3ProjectArchiveBtn) {
    ...
    els.v3ProjectArchiveBtn.hidden = true;
  }
...
function v3ProjectOutputsForJob(jobId, project = v3State.currentProject) {
  ...
  const items = v3StoredProjectOutputItems(project).filter((item) => {
    const metadata = item?.metadata || {};
...
function v3RecoveredLatestVisibleProjectOutputs(project = v3State.currentProject, baseJob = v3State.currentJob) {
  ...
  const latestJobOutputs = latestJobId
    ...
    : [];
  const outputs = v3DeliveryDisplayItems(latestJobOutputs.length ? latestJobOutputs : visibleOutputs);
  if (!outputs.length) return null;
  const inferredJobId =
...
function v3OutputVisibleInProject(item, project = v3State.currentProject) {
  ...
  if (state === "unselected" || state === "rejected") return false;
  if (!v3CanonicalFinalDelivery(item)) return false;
  const identity = v3OutputItemIdentity(item);
  return !v3ReviewOutputIdSet("hidden_output_ids").has(identity);
}

function v3OutputDeliveryState(item) {
  const state = item?.delivery_state || item?.metadata?.delivery_state || "";
...
function v3CanonicalFinalDelivery(item) {
  if (!item || item?._v3Source === "selected_ref") return false;
  ...
  if (v3OutputDeliveryState(item) !== "final_delivery") return false;
  return v3OutputImageCandidates(item).length > 0;
}
...
function v3ProjectOutputReason(item) {
  const rawReason = item?.selection_reason || item?.recommendation || item?.purpose || item?.project_goal || "";
...
function renderV3ProjectWorkflow() {
  ...
  if (v3State.selectedScenario === "ecommerce") {
    ...
    const steps = [
      {
        label: "准备商品信息",
        text: hasProduct ? "已准备商品图或商品参考" : "建议上传一张清晰商品图",
        state: hasProduct ? "done" : project ? "active" : "todo",
      },
      {
        label: "说明需求",
        text: project ? "已保存本次想要的画面" : "用一句话说明想呈现什么效果",
        state: project ? "done" : "todo",
      },
      {
        label: "整理画面方向",
        text: "系统会根据本次商品、需求和补充信息决定每张完整图片",
        state: project ? "done" : "todo",
      },
      {
        label: "生成图片",
        text: hasGenerated ? "已有可查看的最终交付" : "生成后会显示每张完整图片",
        state: hasGenerated ? "done" : project ? "active" : "todo",
      },
      {
        label: "查看和继续",
        text: hasGenerated ? "查看、下载或继续制作满意的图片" : "生成后再查看最终交付",
...
function v3ProjectSubpageCopy(stepKey = "compose") {
  ...
  const map = {
    compose: {
      eyebrow: "继续生产",
      title: ecommerceActive ? "继续制作商品图片" : "继续生成图片",
      intro: ecommerceActive ? "写清这次想呈现的画面，按需补充商品图和事实，再选择数量。" : "写一句目标，可补参考图，V3 会沿用本项目方向出图。",
    },
    review: {
      eyebrow: "查看结果",
      title: "查看这次生成的图片",
...
function renderV3ContinueScene() {
  ...
  return renderV3SceneRows([
    ...
    },
    {
      ...
      hint: isEcommerce ? "适合后续同商品或同店铺继续制作。" : "适合之后的新项目继续沿用这组视觉感觉。",
    },
  ]);
}
...
function renderV3ProjectNextActions() {
  ...
  if (sourceAnalysisUnavailable) {
    ...
    title = "本次图片已完成";
    detail = expectedCount && visibleCount !== expectedCount
      ? `当前已交付 ${visibleCount} 张；请求的是 ${expectedCount} 张。项目已保留，请查看状态后再继续。`
      : `当前已交付 ${visibleCount} 张完整图片。可以查看、下载或继续制作。`;
    actionRows.push(
      '<button class="button primary compact" type="button" data-v3-project-action="show_project_results">查看图片</button>',
      '<button class="button secondary compact" type="button" data-v3-project-action="continue_same_style">继续制作</button>',
      '<button class="button ghost compact" type="button" data-v3-project-action="return_to_project">返回项目</button>',
    );
  } else {
    title = ecommerceActive ? "准备生成商品图片" : "准备生成图片";
    detail = ecommerceActive
      ? "上传商品参考图、写清需求并选择数量后即可开始；补充信息始终是可选的。"
      : "写清需求后即可开始生成。";
    actionRows.push(
      `<button class="button primary compact" type="button" data-v3-project-action="start_first_generation">${ecommerceActive ? "继续填写并生成" : "继续生成"}</button>`,
      `<button class="button secondary compact" type="button" data-v3-project-action="${ecommerceActive ? "upload_reference_continue" : "continue_same_style"}">${ecommerceActive ? "上传商品图" : "继续创作"}</button>`,
      '<button class="button ghost compact" type="button" data-v3-project-action="return_to_project">返回项目</button>',
    );
  }
  els.v3ProjectNextActions.innerHTML = `
    <div class="v3-continuation-panel">
      <div class="v3-continuation-main">
        ...
        <div class="v3-continuation-buttons">${actionRows.join("")}</div>
      </div>
    </div>
  `;
  els.v3ProjectNextActions.hidden = false;
...
function renderV3BrandMemoryPanel() {
  ...
  // Brand-memory management is intentionally secondary to the current
  // project loop. It remains available through its dedicated route/modal,
  // but does not compete with generation, selection, or continuation here.
  els.v3BrandMemoryPanel.hidden = true;
...
function handleV3ProjectActionClick(event) {
  ...
  if (action === "edit_ecommerce_details") {
    ...
    els.v3EcommerceSpecsInput?.focus?.({ preventScroll: true });
  ...
  if (action === "upload_reference_continue") {
    if (els.v3PromptInput && !els.v3PromptInput.value.trim()) {
      els.v3PromptInput.value = v3State.selectedScenario === "ecommerce"
        ? (goal || "参考我上传的商品图，继续制作同项目风格的完整商品图片。")
        : (goal || "参考我上传的图片，继续生成同项目风格的图片。");
    }
    if (els.v3AssetInput) els.v3AssetInput.click();
    updateV3Notice(v3State.selectedScenario === "ecommerce" ? "可以上传或更换商品图。参考图会留在这个电商项目里。" : "可以上传新参考图，这次生成会参考它；满意后可选为项目方向。", "info");
    return;
  }
...
function handleV3ProjectClick(event) {
  ...
  if (actionButton) {
    ...
    if (actionButton.dataset.v3ProjectAction === "load_more_projects") {
      loadV3Projects({ silent: false, loadMore: true }).catch((error) => {
        showGlobalToast(`更多项目加载失败：${friendlyError(error)}`, "warning");
      });
      return;
    }
...
function handleV3HistoryClick(event) {
  if (event.target.closest("[data-v3-history-load-more]")) {
    event.preventDefault();
    event.stopPropagation();
    loadV3Projects({ silent: false, loadMore: true }).catch((error) => {
      showGlobalToast(`更多项目加载失败：${friendlyError(error)}`, "warning");
...
function openV3VisualAssetLibraryDialog({ focusBuilder = false } = {}) {
  ...
  window.setTimeout(() => {
    els.v3VisualAssetCreateForm?.scrollIntoView?.({ behavior: "smooth", block: "start" });
    els.v3VisualAssetNameInput?.focus?.({ preventScroll: true });
  }, 0);
}
...
function isV3VisualAssetImageFile(file) {
  const mediaType = String(file?.type || "").toLowerCase();
  const filename = String(file?.name || "").toLowerCase();
  return /^(image\/(png|jpeg|webp))$/.test(mediaType) || /\.(png|jpe?g|webp)$/.test(filename);
}

function clearV3VisualAssetSourcePreviews() {
  (Array.isArray(v3State.visualAssetSourcePreviewUrls) ? v3State.visualAssetSourcePreviewUrls : []).forEach((url) => {
    if (url) URL.revokeObjectURL(url);
  });
  v3State.visualAssetSourcePreviewUrls = [];
}

function clearV3VisualAssetSourceFiles() {
  clearV3VisualAssetSourcePreviews();
...
function handleV3VisualAssetSourceFiles(event) {
  const selected = Array.from(event?.target?.files || []).filter(isV3VisualAssetImageFile);
...
function handleV3VisualAssetSourceListClick(event) {
  ...
  if (button.dataset.v3VisualAssetSourceAction === "remove") {
    ...
    v3State.visualAssetSourcePreviewUrls = previewUrls;
...
function renderV3VisualAssetSourceFiles() {
  ...
  files.forEach((file, index) => {
    ...
    const previewUrl = Array.isArray(v3State.visualAssetSourcePreviewUrls) ? v3State.visualAssetSourcePreviewUrls[index] : "";
    const row = document.createElement("div");
...
async function v3UploadVisualAssetRoot(file) {
  ...
  await request(created.upload_url || `${v3ApiBase}/uploads/${encodeURIComponent(created.asset_id)}/content`, {
    method: "PUT",
    body: { content_base64: contentBase64, mime_type: file.type || created.mime_type || "image/png" },
  });
...
function handleV3AssetListClick(event) {
  const button = event.target.closest("[data-v3-remove-upload]");
  if (!button) return;
  removeV3AssetFile(Number(button.dataset.v3RemoveUpload));
}

function removeV3AssetFile(index) {
  if (!Number.isInteger(index) || index < 0 || index >= v3State.files.length) return;
  v3State.files = v3State.files.filter((_, itemIndex) => itemIndex !== index);
  v3State.uploadedAssets = [];
  v3State.uploadFingerprints = {};
  if (els.v3AssetInput) els.v3AssetInput.value = "";
  renderV3Assets();
  updateV3Notice(
    v3State.files.length
      ? `已移除这张参考图，当前还有 ${v3State.files.length} 张。`
      : "参考图已清空。",
    "info",
  );
}
  ...
  v3State.files.forEach((file, index) => {
    ...
    row.innerHTML = `
      <span class="v3-asset-file-copy">
        ...
        <small>${escapeHtml(v3FileSizeText(file.size))}</small>
      </span>
      <button type="button" data-v3-remove-upload="${index}" aria-label="移除这张参考图">移除</button>
    `;
...
function v3UploadedReferenceUsePolicy(asset, isEcommerce = false) {
  if (isEcommerce || asset?.role === "product_reference") return "product";
  const text = [
    asset?.role,
    asset?.filename,
    els.v3PromptInput?.value,
  ...
  if (asset?.role === "face_reference" || asset?.role === "portrait_identity" || v3LooksLikeHumanReferenceTask(text)) {
    return "identity";
  }
  return "general";
}
...
async function maybePersistV3UploadedReferences(uploadedAssets = []) {
  ...
  for (const asset of assets) {
    await request(`${v3ApiBase}/projects/${encodeURIComponent(projectId)}/references`, {
      ...
      body: {
        ...
        use_policy: v3UploadedReferenceUsePolicy(asset, isEcommerce),
        metadata: {
          frontend_surface: "commercial_v3_project_mode",
  ...
  await refreshV3CurrentProject({ silent: true });
  updateV3Notice(isEcommerce ? "商品图已保存到这个项目，后续电商图片会继续参考。" : "参考图已保存到这个项目，后续生成会继续沿用。", "success");
}
...
function v3EcommerceProfilePatch() {
  ...
  const sellingPoints = v3TextLines(els.v3EcommerceSellingPointsInput?.value || "");
...
function v3DeclaredGenerationCounts(templateId = v3State.selectedTemplate) {
  ...
  const parsed = Array.isArray(declared)
    ? declared.map((value) => Number.parseInt(String(value), 10)).filter((value) => Number.isFinite(value) && value > 0)
    : [];
  if (parsed.length) return [...new Set(parsed)].sort((left, right) => left - right);
  return templateId === "ecommerce_template" ? [...v3EcommerceExactCountContract] : [1, 2, 3, 4];
}

function v3BoundedGenerationCount(value, scenarioId = v3State.selectedScenario) {
  const number = Number.parseInt(String(value ?? ""), 10);
  const templateId = v3TemplateIdForScenario(scenarioId || "general_creative");
  const supported = v3DeclaredGenerationCounts(templateId);
  if (!Number.isFinite(number)) return supported.includes(2) ? 2 : supported[0] || 1;
  return supported.includes(number) ? number : (supported.includes(2) ? 2 : supported[0] || 1);
}
...
function syncV3GenerationCountControl() {
  ...
  const scenarioId = v3State.selectedScenario || "general_creative";
  const supported = v3DeclaredGenerationCounts(v3TemplateIdForScenario(scenarioId));
  const requested = v3BoundedGenerationCount(input?.value || v3State.generationCount || 2, scenarioId);
  if (input && input.tagName === "SELECT") {
    input.innerHTML = supported
      .map((count) => `<option value="${count}">${count} 张</option>`)
      .join("");
  }
  if (fixedCount === null) {
    if (input) {
      v3State.generationCount = requested;
      input.value = String(requested);
      input.disabled = false;
      input.removeAttribute("aria-label");
    }
    if (els.v3CountValue) els.v3CountValue.textContent = String(requested);
    if (els.v3CountHint) {
      els.v3CountHint.textContent = scenarioId === "ecommerce"
        ? `本次准备生成 ${requested} 张完整图片；不支持时会明确提示，不会少生成。`
        : `本次准备生成 ${requested} 张图片。`;
    }
    return;
  }
  ...
  if (els.v3CountHint) els.v3CountHint.textContent = `本次准备生成 ${fixedCount} 张图片。`;
}
...
async function selectV3Job() {
  ...
  try {
    ...
    if (selected?.metadata?.selection_held) {
      if (selected.job_status) v3State.currentJob = selected.job_status;
      if (selected.project) v3State.currentProject = selected.project;
      syncV3ProjectOutputsFromPayload(selected);
      renderV3Job(v3State.currentJob);
      renderV3ProjectDetail();
      updateV3Notice(selected.warnings?.[0] || "这张图暂时还不能安全作为后续参考。", "warning");
      return;
    }
    v3State.selectedResult = selected;
    v3State.activeProjectStep = "compose";
...
function resetV3Workspace() {
  ...
  if (els.v3EcommerceSellingPointsInput) els.v3EcommerceSellingPointsInput.value = "";
...
function ensureV3PageLoadingOverlay() {
  let overlay = document.querySelector("#v3PageLoadingOverlay");
  if (overlay) return overlay;
  overlay = document.createElement("div");
  overlay.id = "v3PageLoadingOverlay";
  overlay.className = "v3-page-loading-overlay";
  overlay.hidden = true;
  overlay.innerHTML = `
    <strong id="v3PageLoadingTitle">正在进入项目</strong>
    <small id="v3PageLoadingDetail">正在读取项目图片、记录和上下文。</small>
  `;
  document.body.appendChild(overlay);
  return overlay;
}

function setV3PageLoading(visible, title = "", detail = "") {
...
function ensureAppPageLoadingOverlay() {
  let overlay = document.querySelector("#appPageLoadingOverlay");
  if (overlay) return overlay;
  overlay = document.createElement("div");
  overlay.id = "appPageLoadingOverlay";
  overlay.className = "v3-page-loading-overlay app-page-loading-overlay";
  overlay.hidden = true;
  overlay.innerHTML = `
    <strong id="appPageLoadingTitle">正在加载</strong>
    <small id="appPageLoadingDetail">正在读取页面数据。</small>
  `;
  document.body.appendChild(overlay);
  return overlay;
}

function setAppPageLoading(visible, title = "", detail = "") {
  ...
  if (detailNode) detailNode.textContent = detail || "正在读取页面数据。";
}
...
function markV3HomePreviewImageFailed(image) {
  if (!image) return;
  image.dataset.v3HomeThumbFailed = "true";
...
function waitForV3HomePreviewImages({ blockPage = true } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const check = () => {
      if (!images.every(v3ImageSettled)) return;
      cleanup.forEach((release) => release());
      window.clearTimeout(timeout);
      resolve();
    };
...
function renderV3EcommerceSummary(summary, metadata = null, job = null) {
  ...
  const entries = [
    ...(failureMessage ? [failureMessage, "项目已保存，不会用固定图组或本地图片代替这次结果。"] : []),
    "已整理商品事实、参考图和不可违背的约束",
    ...(summary?.platform ? [`已附上 ${summary.platform}${summary.market ? ` / ${summary.market}` : ""} 的发布要求，发布前仍建议核验最新规则`] : []),
    ...(categoryLabel ? [`已补充 ${categoryLabel} 类目的商品事实`] : []),
    ...(outputIntents.length ? [`本次已整理 ${outputIntents.length} 个完整画面方向`] : ["会根据本次商品和目标整理完整画面方向"]),
    ...(copyPlanningLabel && approvedCopy ? [`已记录需要按 ${copyPlanningLabel} 使用的原文文案`] : []),
    ...(targetAudience.length ? [`已优先考虑：${targetAudience.join(" / ")}`] : []),
    "每张图片都由本次商品、需求和补充信息共同决定",
  ];
  ...
  if (productionGatePassed) entries.push("当前项目结果已记录为可交付状态。");
  renderV3OutcomeItems(entries);
  renderV3ClosureChecks([]);
...
function renderV3EcommercePlanList(summary) {
  ...
  intents.slice(0, 6).forEach((intent, index) => {
    ...
    const outputLabel = v3EcommerceOutputLabel(intent.output_id, index + 1);
    const purpose = intent.intent || "正在整理这张完整图片的画面方向";
    row.innerHTML = `
...
function renderV3EcommerceExportList(summary) {
  ...
  const fileRows = files.slice(0, 6).map((file, index) => {
    const outputLabel = v3EcommerceOutputLabel(file.opaque_output_id, index + 1);
  ...
  els.v3EcommerceExportList.innerHTML = `
    <div class="v3-commerce-export-row">
      ...
      <span>${escapeHtml(metadata.publish_summary || `${files.length} 张图片已规划`)} · ${escapeHtml(profile)}</span>
    </div>
    ${fileRows.join("")}
    ${checkRows.join("")}
  `;
...
function renderV3ResultBoard(job) {
  ...
  if (scenarioId === "ecommerce") {
    ...
    summary.className = "v3-result-delivery-summary";
    summary.textContent = items.length === expectedCount
      ? `已交付 ${items.length} 张完整图片。`
      : `当前已交付 ${items.length} 张；本次请求 ${expectedCount} 张，未交付的图片不会被当作完成。`;
    els.v3ResultBoard.appendChild(summary);
  }
  v3State.resultItems.slice(0, 8).forEach((item, index) => {
    ...
    `;
    const image = card.querySelector(".v3-result-preview img");
    const showImageRecovery = () => {
      if (scenarioId !== "ecommerce" || card.querySelector(".v3-image-recovery")) return;
      const recovery = document.createElement("div");
      recovery.className = "v3-image-recovery";
      recovery.innerHTML = `
        <span>图片暂时无法加载。</span>
        <button class="button compact ghost" type="button" data-v3-image-reload>重新加载图片</button>
...
function openV3OutputLightbox(item, { title = "V3 生成图片", meta = "", promptText = "", promptOpen = false } = {}) {
  const metadata = item?.metadata || {};
  const templateId = item?.template_id || metadata.template_id || v3State.currentProject?.primary_template_id || "";
  const suppressPrompt = templateId === "ecommerce_template";
  ...
  openImageLightbox({
    ...
    meta,
    promptText: suppressPrompt ? "" : (promptText || metadata.final_prompt || metadata.optimized_prompt || ""),
  });
  if (promptOpen && els.lightboxPromptPanel?.hidden) toggleLightboxPrompt();
}
...
async function initV2({ silent = true, force = false } = {}) {
  ...
  if (showPageOverlay) setAppPageLoading(true, "正在加载 V2", "正在读取历史、模板和中枢状态。");
  ...
  try {
    ...
    if (showPageOverlay) setAppPageLoading(false);
  }
}
...
function setVeyraToken(token) {
  ...
  if (nextToken !== previousToken) clearV3LocalCaches();
  try {
    if (nextToken) {
...
function setVeyraAccount(account) {
  const previousOwnerId = v3CacheOwnerId();
  veyraState.account = account;
  const nextOwnerId = v3CacheOwnerId(account);
  if (previousOwnerId && nextOwnerId && previousOwnerId !== nextOwnerId) clearV3LocalCaches();
}
...
function veyraLoginUrl(target = "alchemy") {
  ...
  const callback = `/_veyra/return?target=${encodeURIComponent(target)}`;
...
async function refreshHistory({ silent = false, append = false } = {}) {
  ...
  const showPageOverlay = !append && (activePanelName() === "image" || !silent);
  if (showPageOverlay) setAppPageLoading(true, "正在加载 V1", "正在读取最近生成记录和缩略图。");
  try {
