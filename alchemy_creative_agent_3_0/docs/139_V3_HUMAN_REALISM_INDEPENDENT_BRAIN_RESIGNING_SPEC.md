...
The defect is not solved by local wording. The correct owner of a substantive
revision remains the remote Brain.
...
- The feature is a shared Human Realism capability step and applies to any
  real visible person.
...
- It returns the same exact-count canonical-provider-prompt contract and the
  ...
  the enforced job before any image operation.

## 3. Required review question

The re-signer silently asks whether the complete candidate makes a particular
person naturally present in the user-owned photographic situation, rather than
a default polished beauty archetype. It must preserve legitimate user-requested
glamour, product truth, age/reference truth and scene mood. It returns no chain
of thought, issue list or visible checklist.

## 4. Audit and retry

The frozen job records that an independent Human Realism re-sign occurred,
which remote Brain/model supplied it, and the safe same-pass Doc142 Human
Naturalness decision receipt
...
7. The Doc142 decision receipt has valid owner, version, cardinality and frozen
   binding, and its absence blocks before Provider/MCP materialization.
