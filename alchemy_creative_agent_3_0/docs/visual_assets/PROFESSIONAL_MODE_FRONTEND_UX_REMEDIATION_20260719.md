...
状态：`superseded_for_forward_ux`

基线：`origin/main@56fbac6`

共享前端原则见 Doc172。本文件记录了原项目级 Professional Mode 界面的修复要求，
不新增 Provider、Brain、Review、Retry 或存储实现。

> **新版权威。** 新建视觉资产、新项目使用资产和前端导航必须遵循 Doc173。这里的
> “创建项目时选择专业模式”、project-scoped People Asset 与对应路由不得继续作为
> 新前端实现目标。保留本文件的状态人话、错误投射、无静默回退、确认与移动端可用性
> 要求，并把它们迁移到“视觉资产库 + 项目绑定”体验中。
...
> 本节所述首页模式切换已被 Doc173 §5.1 取代：专业能力的前向入口是独立的
> “视觉资产库”，项目中通过“选择视觉资产”进行可选绑定。
...
> 下列 project-scoped route 仅用于旧项目读取与显式迁移；新的浏览器调用应采用
> Doc173 §4 的 library asset API 与 project binding API。
...
> 新手正向路径改为 Doc173 §1.3/§5.2/§5.3 的“先建立资产、再在项目中绑定”。
...
> 其中 project-scoped binding 的断言替换为 library ownership、project binding、Job
> freeze、旧记录只读兼容和无资产 Standard path 的断言，具体见 Doc173 §8。

- V3 首页能发现 Professional，但 Standard/General 不出现专业字段；
- 未选择 Professional 时，人物上传仍走普通参考图语义；
- 创建或绑定 People Asset 后，刷新、返回、重开仍保持 library asset 与项目 binding
  的服务端真相；
