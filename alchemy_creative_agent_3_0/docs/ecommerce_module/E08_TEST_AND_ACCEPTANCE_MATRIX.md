# E08 Test and Acceptance Matrix

## Context and contract tests

- platform evidence has version, status, market, source metadata, and no executable output map;
- stale/unknown evidence warns without inventing a generic suite;
- category context contains buyer questions/review cues, not a visual recipe;
- new jobs contain no local role, slot, camera, crop, scene, or default-copy direction;
- protected facts and approved literal copy remain unchanged;
- export records profile/category evidence versions and opaque output IDs;
- public requests reject provider-level controls and retired static slot input.

## Remote-Brain tests

- E-Commerce factual context reaches only the E-Commerce Brain payload;
- requested count is returned exactly as one natural-language intent per output;
- an unavailable, fallback, or incomplete remote Brain plan fails closed in production;
- explicit labelled test doubles are the only test exception;
- the vertical pack binds Brain intents to opaque IDs but never fills a missing purpose, role, or scene;
- General and Photography payloads/role plans contain no E-Commerce context.

## Isolation tests

- no V1/V2/Lab import or storage access;
- no provider call from platform/category evidence code;
- no marketplace terms in General Template UI or prompt defaults;
- shared Human Realism/Product Identity stays in the shared cluster;
- historical jobs remain readable but cannot restart through static planning;
- retired text-pixel inputs return a provider-native-required result and cannot reach a font, OCR, canvas, compositor, or private retry loop.

## Review, retry, and export tests

- product drift, unsupported claims, fake certificates, and invented specs are observed as warnings/failures;
- verified text constraints and approved copy reach the Brain/provider;
- final-pixel issues use only bounded shared provider-native revision;
- retry-superseded results are folded from delivery;
- the best reviewed candidate wins over the newest candidate;
- output IDs remain stable across continuation and failed children preserve the prior current delivery.

## UI tests

- a beginner can upload a product, choose factual context, set count, and generate;
- advanced factual controls are hidden initially;
- output cards show image, Brain-returned purpose, and permitted actions;
- no suite scope, role selector, static slot preview, overlay-copy mode, or local typography control is rendered;
- one opaque output can continue without disturbing other current results;
- General Template renders none of the E-Commerce UI;
- mobile board remains image-first.

## Real-output fixture matrix

Run real product references across the first release categories and, where supported by current sourced evidence, Amazon/en-US, Ozon/ru-RU, Taobao-or-Tmall/zh-CN, and TikTok-Shop/market-equivalent. Inspect product truth, exact approved text when requested, language, hands or models when present, scene realism, useful differentiation, verified platform constraints, output count, export names, and append-only retry history.

## Activation gate

E-Commerce may be marked active only when E00-E08 and E17 are accepted; all context, Brain, isolation, review, retry, UI, and full V3 regressions pass; real provider fixtures pass manual review; project/reference boundaries are enforced; evidence versions are frozen; General/Photography remain unaffected; and no platform-compliance or typography guarantee is claimed.
