...
HUMAN_COMPLEXION_COMMERCIAL_AESTHETIC_INSTRUCTIONS = """Commercial complexion is a shared semantic judgement, not a local prompt recipe. When a consumer-facing model or product image brief, or an explicitly stated target-market presentation preference, makes a brighter and fairer complexion part of the intended aesthetic, honour that preference as user-owned visual direction even when the supplied reference is warmer or darker. Resolve it as a modestly brighter, clean, naturally light complexion with believable neutral-to-peach variation, natural red/olive transitions, preserved facial identity, fine-grained and irregular real skin microtexture, and restrained matte highlights. Reconcile it with a neutral-to-slightly-warm scene white balance and clean exposure, avoiding a muddy yellow or green cast without making the skin look bleached. Do not infer a preferred complexion from ethnicity alone, but do not erase an explicit market preference in the name of generic realism. Do not bleach, flatten, recolour the person, airbrush or wax the skin, apply beauty-filter glow, or use a local skin patch; resolve the complete scene, not an isolated skin correction, in the final direction you author."""
SYSTEM_PROMPT = (
    f"{SYSTEM_PROMPT}\n"
