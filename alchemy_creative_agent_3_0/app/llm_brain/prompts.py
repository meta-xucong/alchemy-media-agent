...
def _canonical_provider_prompt_finalization_payload(request: BrainRunRequest) -> dict[str, object]:
    ...
    if presence_decision_required:
        response_contract += (
            ...
            "Do not approve a generic stage label, vague naturalness adjective, or iconic stage-feature list as sufficient "
            "resolution. Reconcile soft-tissue response, attention and affect as one integrated person in the exact scene "
            "without prescribing facial anatomy, measurements, a demographic template or a fixed expression. A quiet neutral "
    ...
    if is_developmental_presence_verify:
        response_contract += (
            ...
            "The candidate wording is not authoritative and must not be preserved for convenience. Reconstruct the complete "
            "renderer direction from the protected user intent, frozen reference ownership and whole-image facts, while retaining "
            "all legitimate user-owned scene, identity, styling and expression choices. Judge the developmental stage as an "
            "integrated perceptual outcome, but keep the diagnostic basis out of renderer language: do not prescribe inferred "
            "facial anatomy, enumerate age cues, or invent mouth, teeth, eye or smile geometry that the user did not request. "
