...
Document 96: high-fidelity portrait identity execution authority. Hard identity edits negotiate high input fidelity and use ephemeral objective identity metrics alongside multimodal review; one bounded local-repair extension point remains available, no biometric vectors are persisted, and Doc97 governs whether the active provider may use that repair path.
Document 97: current subject-continuity asset-pack and adaptive-reference authority. Explicit user selections lead the next generation while uploaded identity truth remains the root guard; weak generated anchors are quarantined, reference evidence is view-aware and bounded, and local face repair requires an explicit identity-native provider capability.
Document 98: current identity-native HTTP sidecar authority. Applicable Doc97 portrait jobs may use a capability-probed external InstantID/PhotoMaker/PuLID-compatible backend; one failed optional sidecar attempt falls back to the existing provider, fallback outputs never claim identity-native delivery, and real quality gains require a deployed sidecar plus controlled A/B acceptance.
Document 99: current deployable identity-sidecar gateway and acceptance authority. The standalone CPU gateway translates Doc98 into a reviewed ComfyUI GPU workflow, requires license and semantic attestations plus live node/health checks, bounds uploads/concurrency/idempotency, and cannot make a production quality claim before the five-view Doc89/Doc96 matrix passes.
