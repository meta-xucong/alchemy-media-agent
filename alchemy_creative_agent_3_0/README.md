...
Documents 50-64: historical implementation and regression material for the native Visual Capability Cluster, review/retry, identity, product consistency, and mode work. Their static suite, role, and prompt-compilation proposals are superseded for new jobs by Docs93, 94, 100, 111, and 113.
...
Document 65: historical precursor for Human Realism observations; Doc91 is the current shared capability governance authority.
...
Document 68: historical casebook record. It must not restore local photographic recipes or prompt stacks.
Documents 69-75: historical prompt-atom, tuning, and suite-director material. Named cases remain regression fixtures only; Docs93, 94, 111, and 113 govern forward execution. Documents 76-78 remain foundation/specialized-template and long-term-quality governance.
...
Document 91: current Human Realism ownership and contribution governance. It is a shared, evidence-driven capability activated only by the frozen plan; it has no child/kidswear branch, prompt catalogue, vertical route, or template role. Identity truth remains owned by the reference/identity modules.
...
Document 96: objective portrait identity metric authority. Its former local-repair production direction is superseded by Doc100; identity evidence remains ephemeral review input.
...
Document 101: current capability-activation and visual-plugin extensibility authority. Registered does not mean enabled; Central Brain proposes capabilities, the shared planner validates manifests/evidence/dependencies/conflicts, and inactive plugins contribute nothing.
Document 102: current file-level implementation and rollout authority for Doc101. New jobs use enforced activation by default: one frozen per-job plan controls selective plugin execution, composed provider input, review, retry, and memory. Existing frozen legacy/shadow plans retain their recorded mode, while an explicit environment setting remains available for a controlled rollback.
Document 111: provider-native creative-direction correction. The remote Central Brain supplies complete natural-language image direction and GPT Image 2 produces the complete image; deterministic text compositing, prompt-atom stacks, static recipes, and local image construction are not forward paths.
Document 113: current execution-truth authority. One normalized intent, frozen envelope, template-owned deliverable plan, resolved ledger, materialization, and truthful review/retry chain govern every V3 job.
Document 114: gated apparel-on-child garment/person quality specification. It has no child-specific runtime; its real gate begins only after actual pixels exist and remains open until the shared/provider evidence chain is complete.
Document 117: real-reference Provider capability and no-pixel failure closure. It adds a derived audit/status projection and precise classifier to the existing lifecycle; generic 4xx responses are not assumed to be policy blocks.
